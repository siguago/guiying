...
- 桌面端同一时间只运行一个扫描任务；扫描根只能由当前原生窗口选择并换取一次性、限时、窗口绑定的随机 token，WebView 不能用路径字符串自行扩大读取范围。支持合作式安全停止、任务隔离和轻量进度事件；未完成阶段不会封印，草稿组不可见。
- `guiying-runtime` 将 core 的不可构造读取证明、volume 的 descriptor/mount 复核和 Store 的不可变观察接成同一当前会话主链；SQLite 只写入每用户应用数据目录，不写目标照片盘。
- Tauri 不再传输整份扫描报告；React 首屏只读取第一组有界结果，重复组、组内成员和问题清单都按游标逐页替换，不在内存里累积整个图库。扫描运行期间禁止第二连接分页，避免错误触发陈旧会话恢复。
...
文件动作授权。Phase 1 下一步是补真正可恢复的持久化暂停/续扫、历史结果入口、只读
JSON/CSV 报告导出和真实外置卷矩阵；M2 才会另行实现并故障注入验证同卷隔离与恢复事务。
