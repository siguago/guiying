use crate::reader::{checked_add, checked_sub, BudgetedReader, ParseError, ParseOutcome};
...
pub(crate) fn parse(reader: &mut BudgetedReader<'_>) -> Result<ParseOutcome, ParseError> {
    ...
    match scan_segments(reader, &mut outcome) {
        Ok(()) => Ok(outcome),
        Err(error) if error.is_structural() => {
            // A malformed marker stream cannot be resynchronized safely, but
            // evidence extracted from earlier complete segments stays valid.
...
fn scan_segments(
    ...
    loop {
        ...
        loop {
            ...
            match code {
                MARKER_EOI | MARKER_SOS => return Ok(()),
                ...
                _ => {
                    ...
                    if code == MARKER_APP1 && payload_length >= 6 {
                        ...
                        if signature == b"Exif\0\0" {
                            ...
                            // A malformed embedded TIFF is isolated to its own
                            // APP1 segment; later segments are still scanned.
                            match tiff::parse_in_range(
                                reader,
                                header_offset,
                                segment_end,
                                TiffOrigin::Jpeg {
