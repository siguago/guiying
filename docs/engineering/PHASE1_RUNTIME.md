...
**互斥语义的精确边界**：上述收敛在每次可迁移打开时无条件执行，不按 lease 时间戳的
新鲜度仲裁——任何可迁移的 `Store` 打开都会把当时的 active lease 判为陈旧并回收。
“同一时间只有一个扫描”的产品级保证来自桌面进程在任何可迁移 Store 打开之前持有的
app-data 进程锁加上进程内单任务门；直接嵌入 runtime/store 库的调用方必须自行持有
等价的进程级独占锁，否则另一个进程里健康运行的扫描会被收敛为 interrupted（方向
fail-closed，不污染证据，但扫描白做）。另外，从建立 store attempt 到第一批证据写入
绑定 session 与 lease 之间存在一个对收敛不可见的窗口；该窗口同样由进程锁与单任务门
覆盖，不构成产品内的并发路径，但属于库级契约的一部分。lease 的终态释放对系统时钟
回拨鲁棒：有效当前时间取“墙钟”与“lease 已记录时间戳”的较大值，与启动收敛同构。
...
实施状态：已完成并通过 Rust 1.77.2、SQLite 迁移、重启恢复、离线篡改与有界分页门禁。
该提交只建立持久化证据与查询能力；文件读取声明在接入 descriptor-bound 扫描适配器前
仍不构成可信 I/O 证明，更不构成任何文件动作授权。
