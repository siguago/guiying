...
- 当前 React 适配层仍会在明确的组、成员与问题硬上限内把分页结果组装为现有工作区模型；超过上限会保留数据库证据并拒绝误展示。真正的列表懒加载是下一前端提交。
- 读取可能由文件系统更新 atime；“只读”在产品文案中仅表示归影不主动修改内容、名称、birthtime 或 mtime。

## 回归门禁

Rust 测试必须覆盖：单任务互斥、取消幂等、最终复核取消点、取消终态、失败序列化、终态确认门、owner 窗口清理、进度节流、游标长度/上下文、活动扫描禁止分页，以及真实临时文件从 runtime 封印到 Store 重开分页。前端测试还需覆盖短暂状态查询失败和恢复；真实阻塞 I/O 需要故障注入环境。
